...
function IntroSection() {
  ...
  useEffect(() => {
    const updateDotRect = () => {
      if (dotRef.current) {
        ...
        console.log('Updated dotRect:', dotRect);
      }
    };
    ...
    window.addEventListener('resize', updateDotRect);
  
    return () => {
      window.removeEventListener('resize', updateDotRect);
    };
  }, []);  
  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      introTimeline
        ...
        .to(bgRef.current, {
          scale : 12.5,
          transformOrigin:`${transformValueX + (0.035 * transformValueX)}px ${transformValueY + (0.55 * transformValueY)}px`,
  ...
  return (
    <div ref={containerRef} className="min-h-[600vh] relative min-w-screen max-w-screen " style={{overflow:"clip"}}>
      <div ref={bgRef} className="sticky top-0 w-full max-h-[100vh] overflow-hidden aspect-video bg-[#d3d6db] z-10 ease-linear">
          <p ref={text1Ref} className="absolute top-10 left-[25%] bg-black bg-clip-text text-transparent text-ultra-90 xl:text-ultra-150 pl-[2%] font-neue-bit whitespace-nowrap overflow-hidden">

            Unlock brilliance,
          </p>
          <div ref={text2Ref} className="absolute flex top-[6.5rem] xl:top-36 left-[25%] text-ultra-90 xl:text-ultra-150 font-neue-bit whitespace-nowrap pb-6 overflow-hidden ">
      ...
      >   
        <div
          ref={keyRef}
          className="absolute top-0 left-0 w-[56%] h-[18%] translate-x-[40%]  scale-[1.25] z-40"
        >
          <Key />
        </div>
        <div
          ref={keyShadowRef}
          className="absolute top-0 left-0 w-[56%] h-[19%] translate-x-[40%]  z-30"
        >
          <KeyShadow />
        </div>
      ...
      <StarfieldSection containerRef={starfieldContainer} />
  </div>
  );
}
